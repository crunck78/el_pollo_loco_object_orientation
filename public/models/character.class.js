...
class Character extends Creature {
  ...
  createStatusBars() {
    this.hitPointsBar = new StatusBar(
      30,
      0,
      CHARACTER_ASSETS['IMAGES_HIT_POINTS_BAR'],
      0
    );
    this.hitPointsBar.setPercentage(this.energy);
    this.coinsBar = new StatusBar(
      30,
      40,
      CHARACTER_ASSETS['IMAGES_COINS_BAR'],
      0
    );
    this.coinsBar.setPercentage(this.coins);
    this.bottlesBar = new StatusBar(
      30,
      80,
      CHARACTER_ASSETS['IMAGES_BOTTLES_BAR'],
      0
    );
  ...
  canLaunch() {
    return (
      this.keyboard.SPACE &&
      !(super.isAboveGround() || this.launching || this.landed)
    );
  }
  ...
  attack() {
    ...
    }, 250); //stops playAttacking after timeout
  }
  ...
  playCharacter(timeStamp) {
    if (super.isKilled()) {
      ...
      this.playLaunch(timeStamp);
    } else if (super.isAboveGround() || this.speedY > 0) {
      this.playAboveGround(timeStamp);
    } else if (super.isMovingHorizontally()) {
  ...
  playDead(timeStamp) {
    ...
    setTimeout(() => {
      this.stopPlay();
    }, this.playAnimationElapse * this.IMAGES['DEAD'].length);
  }
  ...
  isStamping(mo) {
    return (
      ...
      super.getHitBoxBottomPos() - mo.getHitBoxTopPos() <= 20
    );
