...
class Enemy extends NPC {
   ...
   */
  patrol() {
    if (this.lastAlertPosition && !this.reachedTarget) {
      //there is a last alert pos reached it,
      if (this.changeDirectionInterval) {
        this.stopDirectionChange();
      } //stop the change direction if its started,
      // stop movement
  ...
  attack(timeStamp) {
    ...
    }, 8 * 300); //attack images length times animationElapse pro attack image
  }
  ...
  alert(target, timeStamp) {
    ...
    if (this.isLeftSide(target)) this.otherDirection = true;
    if (this.isRightSide(target)) this.otherDirection = false;
    ...
    }, 8 * 300); //alert images length times animationElapse pro alert image
  }
