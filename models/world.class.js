...
class World {
  ...
  checkProgress() {
    ...
    if (result) {
      ...
      setTimeout(() => {
        this.level.endScreen =
          result == 'WIN' ? this.level.winScreen : this.level.loseScreen;
      }, 1000);
    }
  }
  ...
  checkCamera(target) {
    ...
    if (
      this.canMoveCamera(
        canvasCenterX,
        this.level.level_end_x - canvasCenterX,
        distanceFromCamera
      )
    ) {
      this.camera_x = -(distanceFromCamera - canvasCenterX);
      if (
        target.isMovingRight() &&
        targetCenterX + this.camera_x >= canvasCenterX * 0.2
      ) {
        this.cameraOffsetX -= 8;
      }
      if (
        target.isMovingLeft() &&
        targetCenterX + this.camera_x <=
          this.ctx.canvas.width - canvasCenterX * 0.9
      ) {
        this.cameraOffsetX += 8;
      }
    }
  ...
  canMoveCamera(leftBoundary, rightBoundary, distanceFromCamera) {
    return (
      distanceFromCamera > leftBoundary && distanceFromCamera < rightBoundary
    );
  }
  ...
  checkAlertEnemies() {
    if (
      ...
      this.level.endBoss.horizontalDistanceFrom(this.level.character) <
        this.level.endBoss.alertDistance
    ) {
   ...
   */
  checkCollisions() {
    this.level.enemies.forEach((enemy, index, enemies) =>
      this.checkEnemyCollisions(enemy, index, enemies)
    );
    this.level.endBoss.chickens.forEach((chicken, index, chickens) =>
      this.checkEnemyCollisions(chicken, index, chickens)
    );
  ...
  checkEnemyCollisions(enemy, index, collection) {
    ...
    this.level.character.throwBottles.forEach((throwObj) =>
      this.checkThrowEnemyCollision(throwObj, enemy, collection)
    );
  }
  ...
  checkEnemyCharacterCollision(enemy, index, collection) {
    if (
      enemy.canCollide() &&
      this.level.character.canCollide() &&
      this.level.character.isColliding(enemy)
    ) {
      if (this.level.character.isStamping(enemy)) {
        this.level.character.stamp(enemy);
        if (enemy.isKilled()) {
          // this should anyways happen
  ...
  checkCollisionsWithCollectibles(collectibles) {
    collectibles.forEach((collectible, index) => {
      if (
        this.level.character.isColliding(collectible) &&
        this.hasCollect(collectible)
      ) {
  ...
  checkCollisionWithPlatforms(target, platforms) {
    for (let index = 0; index < platforms.length; index++) {
      ...
      if (platform.isUnder(target)) {
        ...
        break;
      } else target.groundPos = this.groundPos - target.height;
    }
  }
  ...
  addToMap(mo) {
    if (mo && this.insideCanvas(mo)) {
      //because of delete use , we ask if mo is defined
  ...
  insideCanvas(mo) {
    ...
    return mo.x + mo.width + translate > 0 && mo.x + translate < CANVAS_WIDTH;
  }
